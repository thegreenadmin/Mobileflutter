...
class StoreHomeMainController extends GetxController {
  ...
  late StoreCategoriesListResponse categoriesListResponse = StoreCategoriesListResponse();
  RxList<Category> categoriesList = <Category>[].obs;
  late FeatureProductListResponse featureProductListResponse = FeatureProductListResponse();
  ...
  Future apiGetStoreCategoriesApi() async {
    ...
    UserProvider()
      ...
      if (value?.body["status"] == 201 || value?.body["status"] == 200) {
        categoriesListResponse = StoreCategoriesListResponse.fromJson(value?.body);
  ...
  Future apiFeatureProductListApi() async {
    ...
    UserProvider()
      ...
      if (value?.body["status"] == 201 || value?.body["status"] == 200) {
        Utility.showToast(value?.body['message']);
        featureProductListResponse = FeatureProductListResponse.fromJson(value?.body);
        featureProductList.value = featureProductListResponse.data?.products??[];
      } else if (value?.body["status"] == 403) {
        Utility.showToast(value?.body['message']);
        SharedPreferenceStorage.clearData();
        await Get.offAll(const StartJourneyScreen());
